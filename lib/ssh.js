        ...
        try {
            ...
            for (let hostNo = 0; hostNo < totalHosts; hostNo++) {
                ...
                const promise = new Promise(async resolve => {
                    ...
                    if (hostNo === 0) {
                        files.push({
                            local: 'scripts/docker-compose-RS-PRIMARY.yml',
                            remote: '/home/' + username + '/docker-compose.yml'
                        });
                        files.push({
                        ...
                        files.push({
                            local: 'scripts/docker-compose-RS-SECONDARY.yml',
                            remote: '/home/' + username + '/docker-compose.yml'
                        });
                    }
            ...
            if (primarySSH) {
                ...
                // TODO: Generate Keyfile instead of using a hardcoded one
                // 0. Fix authentication
                // 0.1. Remove root auth setup replica set and setup auth
                // 0.2. Or find out how to login with replica set
                // 1. generate base64 key in node
                // 2. send it as a parameter to every replica
            }
            else {
