...
module.exports = {
    ...
    getReplicaSetStatus: async (username, instance, token) => {
        try {
            const query = `query ReplicaSetStatus {
            		getReplicaSetStatus {
            				set
            				members {
            						_id
            ...
            const res = await graphqlRequest(query, {}, '/api/v1/' + username + '/' + instance, token);
            return res.data.getReplicaSetStatus;
        }
        catch (err) {
            logger.log('error', err);
            return false;
