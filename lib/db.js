            ...
            for (const accountNo in accounts) {
                ...
                if (account.username === username) {
                    ...
                    if (dbList && dbList.length > 0) {
                        ...
                        console.log(newDB);
                        console.log(newDB !== undefined);
                        if (newDB !== undefined) {
                            dbList.push(newDB);
                        }
                    }
                    else {
                        throw new Error('Cannot find instance ' + instance + ' for user ' + username);
                    }

                    console.log(dbList.length, account.databases.length);
                    if (dbList.length > account.databases.length) {
                        const mutation = `mutation AddDatabase ($username: String, $token: String, $databases: [DatabaseInput]) {
                            updateAccounts (filter: {username: $username, token: $token}, update: {databases: $databases}) {
                                username
                            }
